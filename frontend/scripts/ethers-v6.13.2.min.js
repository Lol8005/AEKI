...
class Transaction {
    ...
    set blobs(_blobs) {
        ...
        for (let i = 0; i < _blobs.length; i++) {
            ...
            if (isBytesLike(blob)) {
                ...
                const proof = hexlify(
                    this.#kzg.computeBlobKzgProof(data, commit)
                );
                blobs.push({
                    data: hexlify(data),
    ...
    #getSerialized(signed, sidecar) {
        ...
        switch (this.inferType()) {
            ...
            case 3:
                return _serializeEip4844(
                    this,
                    sig,
                    sidecar ? this.blobs : null
                );
        }
        ...
        const waiter = new Promise((resolve, reject) => {
            ...
            if (timeout > 0) {
                const timer = setTimeout(() => {
                    cancel();
                    reject(
                        makeError("wait for transaction timeout", "TIMEOUT")
                    );
                }, timeout);
                cancellers.push(() => {
                    clearTimeout(timer);
            ...
            const addAddress = (addr) => {
                if (isHexString(addr)) {
                    ...
                    promises.push(
                        (async () => {
                            addresses.push(
                                await resolveAddress(addr, provider)
                            );
                        })()
                    );
                }
